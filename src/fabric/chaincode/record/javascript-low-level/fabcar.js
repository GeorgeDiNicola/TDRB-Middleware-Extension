...
let Chaincode = class {
  ...
  async queryRecord(stub, args) {
    if (args.length != 1) {
      throw new Error('Incorrect number of arguments. Expecting recordHash ex: CAR01');
    }
    let recordId = args[0];
  ...
  async initLedger(stub, args) {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      await stub.putState('RECORD' + i, Buffer.from(JSON.stringify(records[i])));
